...
template <> struct foo<answer<true>> { template <int I> using typen = X<I>; };
template <> struct foo<answer<false>> { static const int typen = 0; };

int main() {
  auto b __attribute__((unused)) =
      foo<IsPrime<4>>::typen<1>(); // Syntax error if not prime
}
